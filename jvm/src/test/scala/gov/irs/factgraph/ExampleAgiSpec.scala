...
/** This test is meant to be illustrative of:
  ...
  *
  * When you write your tests, ensure you are testing the logic. These tests are testing the logic, but the logic boils
  * down to straight math. Your tests will likely be more involved.
  *
  * Run the following command from the root `fact-graph` `sbt "factGraphJVM/testOnly *ExampleAgiSpec"`
  *
...
class ExampleAgiSpec extends AnyFunSuite {
  ...
  test(
    "calculates AGI correctly given positive wage, 1099, and interest income values, minus non-negative adjustments",
  ) {
