...
object OptionConfigTrait {
  // TODO clean these types up
  def fromXml(node: scala.xml.Node): Seq[OptionConfigTrait] =
    var attrs = node.attributes.asAttrMap + (("value", node.text)) - "collections"
    attrs = attrs.map((key, value) => (key, value.trim))

    CommonOptionConfigTraits.create(attrs.toSeq)
}
